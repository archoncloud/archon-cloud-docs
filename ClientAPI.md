# Archon Cloud Gateway S3 Client Application Programming Interface

This document describes the application programming interface (API) used by the Archon Cloud Gateway platform. The platform attempts to emulate Amazon's AWS S3 as much as possible, but only a small subset of S3's functionality is supported.
...
For example, linux users can use the curl command like this:
    ...
    curl -T files/books/dracula.txt 'https://s3.archon.cloud/sample-bucket/dracula.txt?token=<api_token>'
...
For example, linux users can use the curl command like this:
    ...
    curl https://s3.archon.cloud/sample-bucket/dracula.txt?token=<api_token>

As long as the API token has read permission for the object, the download should be successful.

## Authentication

To authenticate requests, you must use Archon Cloud Gateway API keys, which come in two types: an access key (which has a public "access key ID" component and a private "secret access key" component), and an API token (which is a private secret, and functions as both an "access key ID" and a "secret access key").

Archon Cloud Gateway is S3-compatible, and therefore follows S3-style authentication. As a result, to authenticate using access keys, add an HTTP authentication header with the following format:
...
In addition to S3-style authentication, Archon Cloud Gateway also accepts authentication by appending an API token to the URL query "token". For example,

    GET https://s3.archon.cloud/sample-bucket/sample-object-key.txt?token=<api_token>
